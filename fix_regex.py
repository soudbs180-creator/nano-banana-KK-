...
# Use a very generous regex to match from the start comment to the closing brace
p1 = r'// ✅ 生成完成后重新获取主卡最新位置.*?console\.log\(\'\[executeGeneration\] Resolving Position.*?if\s*\(finalPos\.x === 0 && finalPos\.y === 0.*?\n\s+\}'
...
# Pattern 2: catch path
p2 = r'// 🚀 \[修复\] 确保错误卡片始终显示.*?const errorNode = \{ \.\.\.currentNode, isGenerating: false, error: err\.message \|\| \'Failed\' \};'
