...
// src/lib/supabase.ts
var supabaseUrl = (import.meta.env.VITE_SUPABASE_URL || "").trim();
var supabaseAnonKey = (import.meta.env.VITE_SUPABASE_ANON_KEY || "").trim();
var hasSupabaseConfig = Boolean(supabaseUrl && supabaseAnonKey);
var supabaseConfigIssue = hasSupabaseConfig ? null : "\u7F3A\u5C11 VITE_SUPABASE_URL \u6216 VITE_SUPABASE_ANON_KEY\uFF0C\u8BA4\u8BC1\u4E0E\u4E91\u540C\u6B65\u529F\u80FD\u5C06\u81EA\u52A8\u964D\u7EA7";
if (!hasSupabaseConfig) {
  console.error("[Supabase] \u914D\u7F6E\u7F3A\u5931\uFF0C\u5DF2\u5207\u6362\u4E3A\u5B89\u5168\u964D\u7EA7\u6A21\u5F0F");
  console.error("[Supabase]", supabaseConfigIssue);
}
var fallbackSupabaseUrl = "https://placeholder.invalid";
var fallbackSupabaseAnonKey = "placeholder-anon-key";
...
var normalizePricingBaseUrl = (baseUrl) => baseUrl.replace(/\/+$/, "");
var WUYIN_DEFAULT_ROOT_URL = "https://api.wuyinkeji.com";
var WUYIN_ASYNC_ENDPOINT_RE = /^\/api\/async\/([a-z0-9_.-]+)$/i;
...
var createFallbackWuyinCatalogItem = (modelId) => ({
  modelId,
  modelName: modelId,
  inputPrice: 0,
  outputPrice: 0,
...
async function fetchProviderPricing(baseUrl, systemAccessToken) {
  try {
    ...
    if (data.data && Array.isArray(data.data)) {
      return data.data.map((item) => ({
        ...
        groupRatio: parseFloat(item.group_ratio) || 1,
...
async function fetchProviderModels(baseUrl, apiKey, format = "openai") {
  try {
    ...
    const geminiAuthMethod = runtime.authMethod;
    const response = await fetch(
      resolvedFormat === "gemini" ? buildGeminiModelsEndpoint(baseUrl, apiKey, geminiAuthMethod) : buildOpenAIEndpoint(baseUrl, "models"),
      {
        method: "GET",
        headers: resolvedFormat === "gemini" ? buildGeminiHeaders(geminiAuthMethod, apiKey, runtime.headerName, runtime.authorizationValueFormat) : buildProxyHeaders(runtime.authMethod, apiKey, runtime.headerName, void 0, runtime.authorizationValueFormat)
      }
    );
    if (!response.ok) {
      return [];
    ...
    if (models.length > 0) {
      return models.map((m) => (m.id || m.model || m.name || "").replace(/^models\//i, "")).filter(Boolean);
    }
    return [];
  } catch (error) {
...
var WUYIN_PRICE_API_PATH = "/themes/DigitalBlue/api?action=api_list";
var normalizeBaseUrl2 = (baseUrl) => baseUrl.replace(/\/$/, "");
var stripHtml = (value) => value.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
...
var extractWuyinDisplayPrice = (item) => {
...
function calculateRequestCost(pricing, inputTokens, outputTokens) {
  ...
  const ratio = pricing.groupRatio || 1;
  return (inputCost + outputCost) * ratio;
}
async function cacheProviderPricing(providerId, pricing) {
  try {
    const { error } = await supabase.from("provider_pricing_cache").upsert({
