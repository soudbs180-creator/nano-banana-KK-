...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Check storage mode first (Always run logic)
      ...
      // Force Storage Modal if not set OR if tutorial not seen (New/Reset User Flow)
      if (!storageMode || !tutorialSeen) {
        setShowStorageModal(true);
      } else {
        // If mode exists and tutorial seen, verify keys
  ...
  const handleGenerate = useCallback(async () => {
    ...
    // [FIX] Use Smart Group Positioning (Left-to-Right) instead of View Center
    const currentPos = findNextGroupPosition();
    ...
    finalReferenceImages.forEach(ref => {
      if (ref.data) {
        import('./services/imageStorage').then(({ saveImage }) => {
          saveImage(ref.id, ref.data!).catch(e => console.warn('Ref save failed', e));
        });
      }
    });
    ...
    const generatingNode: PromptNode = {
      ...
      mode: config.mode,
      // Store grounding config? The type might not support it yet, assuming false or update type later.
      // CanvasContext schema update didn't explicitly add enableGrounding to PromptNode interface usually.
      // But we can check types.ts later. For now assume basic params.
    };
  ...
  }, [config, addPromptNode, activeCanvas, activeSourceImage, canvasTransform, findSmartPosition, getPromptHeight, executeGeneration]);
    ...
    const runLayout = () => {
      ...
      const getImageDimsForLayout = (img: any) => {
        ...
        if (typeof img.dimensions === 'string' && /^\d+\s*x\s*\d+$/.test(img.dimensions)) {
          const [w, h] = img.dimensions.split('x').map(s => parseInt(s.trim(), 10));
          if (w > 0 && h > 0) {
            const ratio = w / h;
