...
const NewApiAdminView: React.FC = () => {
    return (
        <div className="rounded-2xl border border-[var(--border-light)] bg-[var(--bg-secondary)] p-6">
            <div className="mb-3 flex items-center gap-3">
                ...
                <div>
                    <h3 className="text-base font-semibold text-[var(--text-primary)]">NewAPI Admin View 已改为只读</h3>
                    <p className="text-sm text-[var(--text-secondary)]">管理信息会继续展示，但写入入口已经下沉到统一解析内核。</p>
                </div>
            </div>
            ...
            <div className="rounded-xl border border-dashed border-[var(--border-light)] bg-[var(--bg-tertiary)] px-4 py-3 text-sm text-[var(--text-secondary)]">
                NewAPI/OneAPI/Suxi 的管理 token、价格同步和渠道目录维护应通过统一设置入口完成，避免单独页面再写第二份配置。
            </div>

            <div className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-indigo-300">
                <ArrowRight size={14} />
                请前往“设置 → API 设置”查看和维护当前渠道。
            </div>
        </div>
    );
};
