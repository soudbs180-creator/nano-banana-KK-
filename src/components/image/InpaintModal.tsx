...
export const InpaintModal: React.FC<InpaintModalProps> = ({ imageUrl, onSave, onCancel }) => {
    ...
    const createAntPattern = useCallback(() => {
        ...
        // 我们在 14x14 区域内画斜线（匹配 8 6 虚线比例）
        pctx.fillStyle = 'rgba(0,0,0,0.85)';
        pctx.fillRect(0, 0, 14, 14);

        pctx.strokeStyle = 'white';
        pctx.lineWidth = 8; // 8px 白色部分
        pctx.lineCap = 'butt';
        pctx.beginPath();
        pctx.moveTo(-7, 7); pctx.lineTo(7, -7);
        pctx.moveTo(0, 14); pctx.lineTo(14, 0);
        pctx.moveTo(7, 21); pctx.lineTo(21, 7);
        pctx.stroke();
    ...
    useEffect(() => {
        const timer = window.setInterval(() => {
            // 28px/s / (1000/60 fps) = 1.68px per frame
