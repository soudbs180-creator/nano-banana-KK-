...
const StorageSettingsView = () => {
    ...
    const loadStats = async () => {
        try {
            ...
            if (state.fileSystemHandle) {
                const { size, count } = await fileSystemService.getFolderUsage(state.fileSystemHandle);
                setLocalUsage(size);
                setLocalCount(count);
            } else {
                setLocalUsage(0);
    ...
    const handleDisconnectLocal = () => {
        if (confirm('确定要断开本地备份文件夹吗？\n断开后，新生成的图片将仅保存在临时缓存中。')) {
            setLoading(true);
    ...
    return (
        <div className="space-y-4 h-full flex flex-col px-1">
            <div className="shrink-0">
                <h3 className="text-xl font-bold flex items-center gap-2 text-left" style={{ color: 'var(--text-primary)' }}>
                    配置储存
                    {isConnectedToLocal && (
                        <div className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                            双重备份已激活
                        </div>
                    )}
                </h3>
            ...
            <div className="flex flex-col gap-3 flex-1 min-h-0">
                ...
                {/* Card 1: 临时缓存 (Temporary Cache) - Compact */}
                <div className="relative p-4 rounded-2xl border border-[var(--border-light)] bg-[var(--bg-secondary)] flex flex-row items-center justify-between shadow-sm shrink-0">
                    {/* Left: Info & Stats */}
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 mb-1">
                            <div className="p-1.5 rounded-lg bg-[var(--bg-tertiary)] text-zinc-500">
                                <Globe size={18} />
                            </div>
                            <h4 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>临时缓存</h4>
                            <div className="flex items-center gap-1 text-zinc-400 text-[10px] font-bold px-2 py-0.5 bg-[var(--bg-tertiary)] rounded-full border border-[var(--border-light)]">
                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                                运行中
                            </div>
                        </div>
                        <div className="flex items-baseline gap-2">
                            <span className="text-xs text-zinc-500">占用空间</span>
                            <span className="text-2xl font-mono font-bold text-[var(--accent-amber)] tracking-tight">
                                {formatBytes(browserUsage)}
                            </span>
                        </div>
                        <p className="text-[10px] text-zinc-500 mt-0.5 max-w-[280px]">
                            浏览器缓存。<span className="text-orange-500/80">空间不足时可能被自动清理。</span>
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col items-end justify-center">
                        <button
                            ...
                            className="px-4 py-2 rounded-xl bg-red-500/5 hover:bg-red-500/10 text-red-400/80 hover:text-red-400 border border-red-500/10 hover:border-red-500/30 transition-all flex items-center gap-2 text-xs font-bold"
                            title="清理浏览器缓存"
                        >
                            <Trash2 size={14} />
                            清理缓存
                ...
                {/* Card 2: 本地备份 (Local Backup) - Compact */}
                <div
                    className={`relative p-4 rounded-2xl border transition-all duration-300 flex-1 flex flex-row items-center justify-between overflow-hidden
                    ${isConnectedToLocal
                            ? 'bg-indigo-600/5 border-indigo-500/50 shadow-[0_0_20px_-10px_rgba(99,102,241,0.2)]'
                            : 'bg-[var(--bg-secondary)] border-[var(--border-light)]'
                        }`}>

                    {/* Left: Info, Stats & Path */}
                    <div className="flex flex-col justify-center gap-2 h-full py-1">
                        <div className="flex items-center gap-2">
                            <div className={`p-1.5 rounded-lg ${isConnectedToLocal ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-[var(--bg-tertiary)] text-zinc-500'}`}>
                                <HardDrive size={18} />
                            </div>
                            <h4 className={`text-base font-bold ${isConnectedToLocal ? '' : 'text-zinc-400'}`} style={{ color: isConnectedToLocal ? 'var(--text-primary)' : undefined }}>本地备份</h4>

                            {isConnectedToLocal ? (
                                <div className="flex items-center gap-1 text-indigo-400 text-[10px] font-bold px-2 py-0.5 bg-indigo-500/10 rounded-full border border-indigo-500/20">
                                ...
                                <div className="text-[10px] font-bold px-2 py-0.5 bg-[var(--bg-tertiary)] rounded-full text-zinc-500 border border-[var(--border-light)]">
                                    未配置
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-6">
                            <div className="flex flex-col">
                                <span className={`text-[10px] font-medium ${isConnectedToLocal ? 'text-indigo-300/70' : 'text-zinc-600'}`}>备份大小</span>
                                <div className={`text-2xl font-mono font-bold tracking-tight ${isConnectedToLocal ? 'text-indigo-400' : 'text-zinc-600'}`}>
                                    {isConnectedToLocal ? formatBytes(localUsage) : '--'}
                                </div>
                            </div>
                            {isConnectedToLocal && (
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-medium text-emerald-300/70">成功备份</span>
                                    <div className="text-2xl font-mono font-bold text-emerald-400 tracking-tight">
                                        {localCount} <span className="text-xs">张</span>
                                    </div>
                                </div>
                            )}
                        </div>

                        <div className="text-[10px] text-zinc-500 max-w-[300px] truncate leading-relaxed">
                            {isConnectedToLocal ? (
                                <span className="flex items-center gap-1 text-indigo-300/80" title={currentFolderName || undefined}>
                                    <Check size={10} /> 路径: {currentFolderName}
                                </span>
                            ) : (
                                "连接本地文件夹，实现原图自动双重备份。"
                            )}
                    ...
                    <div className="flex flex-col gap-2 h-full justify-center min-w-[120px]">
                        {isConnectedToLocal ? (
                            <>
                                <button
                                    onClick={(e) => { e.stopPropagation(); setLoading(true); changeLocalFolder().finally(() => setLoading(false)); }}
                                    className="w-full py-2 bg-[var(--bg-tertiary)] hover:bg-white/5 text-zinc-300 rounded-xl text-xs font-bold transition-all border border-[var(--border-light)] text-center"
                                >
                                    更改位置
                                </button>
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="flex-1 py-2 bg-red-500/5 hover:bg-red-500/10 text-red-400 rounded-xl text-xs font-bold transition-all border border-red-500/10 hover:border-red-500/30 flex items-center justify-center gap-1"
                                        title="断开"
                                    >
                                        断开
                                    </button>
                                    <button
                                        ...
                                        className="px-3 py-2 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 rounded-xl text-xs font-bold transition-all border border-indigo-500/20 flex items-center justify-center"
                                        title="刷新"
                                    >
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="w-full py-3 rounded-xl text-xs font-bold flex flex-col items-center justify-center gap-1 transition-all bg-[var(--bg-tertiary)] text-zinc-300 hover:bg-indigo-600 hover:text-white hover:shadow-lg border border-[var(--border-light)] hover:border-indigo-500"
                            >
                                <FolderOpen size={18} />
                                选择文件夹
                            </button>
                ...
                {/* 🚀 OPFS存储卡片 (仅移动端显示) - Keeping logic but compact if needed, though mostly hidden on desktop */}
                {isMobile && supportsOPFS && (
                    <div className="relative p-4 rounded-xl border border-emerald-500/50 bg-emerald-600/5 flex flex-row items-center justify-between shrink-0">
                        <div className="flex flex-col gap-1">
                            <h4 className="text-sm font-bold text-[var(--text-primary)]">私有文件系统 (OPFS)</h4>
            ...
            {/* Loading Overlay */}
            {loading && (
                <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 rounded-[24px]">
