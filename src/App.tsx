          ...
          <div className="glass-strong p-8 rounded-3xl shadow-2xl max-w-lg w-full relative">
            ...
            <div className="mt-6 space-y-3">
              <button
                onClick={() => {
                  saveApiKeys(apiKeys);
                  setIsUsingFreeKey(false); // Disable free mode when manual keys are saved
                  setShowApiModal(false);
                }}
                className="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-medium py-2 rounded-xl transition-all text-sm mb-3"
              >
                保存密钥
              </button>
              ...
              {/* Free Tier Test Button - DISABLED */}
              <div className="pt-4 border-t border-white/5">
                <button
                  disabled={true}
                  className="w-full bg-zinc-800/50 text-zinc-500 font-medium py-2 rounded-xl text-xs flex items-center justify-center gap-2 border border-white/5 cursor-not-allowed opacity-50"
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
