...
const LEGACY_STORAGE_MODE_MAP: Record<string, StorageMode> = {
    native: 'local',
    indexeddb: 'browser',
};

function normalizeStorageMode(raw: string | null): StorageMode | null {
    if (!raw) return null;
    if (raw === 'local' || raw === 'browser' || raw === 'opfs') return raw;
    return LEGACY_STORAGE_MODE_MAP[raw] || null;
}
...
export async function getStorageMode(): Promise<StorageMode | null> {
    ...
    try {
        ...
        const normalized = normalizeStorageMode(stored);

        // Migrate legacy values from old versions.
    ...
    try {
        ...
        return new Promise((resolve) => {
            ...
            request.onsuccess = async () => {
                ...
                try {
                    // Only check, do not actively request permission outside user gesture.
