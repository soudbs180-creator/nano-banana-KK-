...
function saveLogs(data) {
  try {
    ...
    localStorage.setItem(STORAGE_KEY2, JSON.stringify(safeData));
  } catch (error) {
...
var STORAGE_KEY2, MAX_ENTRIES, listeners;
var init_systemLogService = __esm({
  "src/services/system/systemLogService.ts"() {
    "use strict";
    STORAGE_KEY2 = "kk_studio_system_logs";
    MAX_ENTRIES = 200;
    listeners = [];
  }
});
...
var adminModelService_exports = {};
__export(adminModelService_exports, {
...
function recordCost(model, imageSize, count, prompt = "", refImageCount = 0, usage, debugMeta, keySlotId) {
  ...
  const todayStr = getTodayString2();
...
async function syncWithCloud() {
  ...
  try {
    ...
    if (data && data.daily_date === getTodayString2()) {
    }
...
var init_keyManager = __esm({
  "src/services/auth/keyManager.ts"() {
    ...
    STORAGE_KEY3 = "kk_studio_key_manager";
    ...
    KeyManager = class {
      ...
      getStorageKey() {
        if (!this.userId) return STORAGE_KEY3;
        return `${STORAGE_KEY3}_${this.userId}`;
      }
      /**
      ...
      */
      ...
      updateProvider(id, updates) {
        ...
        this.globalModelListCache = null;
        this.notifyListeners();
        if (updates.baseUrl !== void 0 && !updates.pricingSnapshot) {
...
__export(modelPricing_exports, {
  ...
  getRefImageTokenEstimate: () => getRefImageTokenEstimate2,
...
var STORAGE_KEY4, DEFAULT_REF_IMAGE_TOKENS, BUILTIN_PRICING, FALLBACK_IMAGE_TOKENS, normalizeModelId2, toNumber3, convertPricing, extractPricingMap, cachedOverrides, loadOverrides, setModelPricingOverrides, mergeModelPricingOverrides, getModelPricing, getRefImageTokenEstimate2, getModelCredits, getImageTokenEstimate, isCreditBasedModel, MODEL_PRICING_STORAGE_KEY;
var init_modelPricing = __esm({
  "src/services/model/modelPricing.ts"() {
    ...
    init_keyManager();
    init_adminModelService();
    STORAGE_KEY4 = "kk_model_pricing_overrides";
    ...
    normalizeModelId2 = (modelId) => modelId.trim().toLowerCase();
    toNumber3 = (value) => {
    ...
    extractPricingMap = (raw) => {
      ...
      if (Array.isArray(raw)) {
        raw.forEach((item) => {
          ...
          if (pricing) map[normalizeModelId2(id)] = pricing;
        });
        return map;
      }
    ...
    setModelPricingOverrides = (input) => {
      ...
      cachedOverrides = map;
      localStorage.setItem(STORAGE_KEY4, JSON.stringify(input));
    };
    mergeModelPricingOverrides = (input) => {
      ...
      localStorage.setItem(STORAGE_KEY4, JSON.stringify(merged));
    };
    ...
    getRefImageTokenEstimate2 = (modelId) => {
      const pricing = getModelPricing(modelId);
    ...
    getImageTokenEstimate = (modelId, size) => {
      ...
      const fallback = FALLBACK_IMAGE_TOKENS[normalizeModelId2(modelId)];
    ...
    MODEL_PRICING_STORAGE_KEY = STORAGE_KEY4;
  }
});
...
// src/services/model/unifiedModelService.ts
var unifiedModelService_exports = {};
__export(unifiedModelService_exports, {
  unifiedModelService: () => unifiedModelService
});
var UnifiedModelService, unifiedModelService;
var init_unifiedModelService = __esm({
  "src/services/model/unifiedModelService.ts"() {
    "use strict";
    init_keyManager();
    init_adminModelService();
    UnifiedModelService = class {
      models = [];
      listeners = [];
      initialized = false;
      async initialize() {
      ...
      getCreditCost(id) {
        ...
        if (lowerId.includes("pro") && lowerId.includes("banana") || lowerId.includes("pro") && lowerId.includes("gemini") && (lowerId.includes("image") || lowerId.includes("preview"))) {
          return 2;
        }
        if (lowerId.includes("banana") || lowerId.includes("gemini") && (lowerId.includes("image") || lowerId.includes("preview"))) {
          return 1;
        }
        return 0;
      }
      ...
      convertGlobalModel(model) {
        const adminModel = model.isSystemInternal ? adminModelService.getModel(model.id) : void 0;
        return {
          id: model.id,
          name: model.name,
          provider: model.provider,
    ...
    unifiedModelService = new UnifiedModelService();
  }
});

// src/components/settings/CreditModelSettings.tsx
...
var ShieldAlert = createLucideIcon("shield-alert", __iconNode4);
...
init_apiConfig();
init_providerStrategy();
async function getCachedPricing(providerId) {
  try {
    const { data, error } = await supabase.from("provider_pricing_cache").select("pricing").eq("provider_id", providerId).single();
...
// src/services/model/adminModelAdvisor.ts
init_modelPricing();
var DEFAULT_CREDITS_PER_USD = 15;
...
var normalizeModelId3 = (value) => String(value || "").trim().replace(/^models\//i, "").split("@")[0].toLowerCase();
var roundCredits = (value) => Math.max(1, Math.ceil(value));
var buildQualityPricing = (qualities, baseCredits) => {
...
var estimateUsdFromBuiltinPricing = (modelId) => {
  const pricing = getModelPricing(modelId);
  ...
  if (typeof standardTokens === "number" && Number.isFinite(standardTokens) && typeof pricing.outputPerMillionTokens === "number" && Number.isFinite(pricing.outputPerMillionTokens)) {
    return standardTokens / 1e6 * pricing.outputPerMillionTokens;
  }
  return null;
};
var estimateUsdFromCachedPricing = (modelId, cachedPricing) => {
  if (!Array.isArray(cachedPricing) || cachedPricing.length === 0) return null;
  const normalizedId = normalizeModelId3(modelId);
  const exactMatch = cachedPricing.find((item) => normalizeModelId3(item.modelId) === normalizedId) || cachedPricing.find((item) => normalizeModelId3(item.modelName) === normalizedId);
...
function detectAdminProviderVendor(input) {
  const haystack = [input.providerId, input.providerName, input.baseUrl, ...input.modelIds || []].map((value) => String(value || "").trim().toLowerCase()).join(" ");
...
function buildAdminModelCreditSuggestion(input) {
  ...
  const recommendedCredits = usdEstimate !== null ? roundCredits(usdEstimate * creditsPerUsd) : roundCredits(input.currentCreditCost);
  return {
    recommendedCredits,
...
var CREDIT_MODEL_SELECT_BASE = "provider_id, provider_name, base_url, api_keys, model_id, display_name, description, endpoint_type, credit_cost, is_active, call_count, color, color_secondary, text_color";
var CREDIT_MODEL_SELECT_WITH_LIMIT = `${CREDIT_MODEL_SELECT_BASE}, max_calls_limit, advanced_enabled, mix_with_same_model, quality_pricing`;
var CREDIT_MODEL_OPTIONAL_COLUMNS = ["max_calls_limit", "advanced_enabled", "mix_with_same_model", "quality_pricing"];
var getCreditModelSelect = (options) => {
...
var getMissingCreditModelColumns = (error) => {
  const haystack = [error?.message, error?.details, error?.hint].map((item) => String(item || "").toLowerCase()).join(" ");
  const looksLikeMissingColumnError = haystack.includes("does not exist") || haystack.includes("could not find") || haystack.includes("column");
  if (!looksLikeMissingColumnError) {
    return [];
  }
  return CREDIT_MODEL_OPTIONAL_COLUMNS.filter((column) => haystack.includes(column));
};
var inferEndpointType = (modelId) => {
  const id = modelId.toLowerCase();
...
var normalizeBaseModelId = (value) => {
  return (value || "").split("@")[0].trim();
};
var normalizeHexColor = (value, fallback = "") => {
...
var formatUsdEstimate = (value) => {
  if (value === null || !Number.isFinite(value)) return "--";
  return value >= 1 ? `$${value.toFixed(2)}` : `$${value.toFixed(4)}`;
};
var areQualityPricingEqual = (left, right, qualities) => qualities.every((quality) => {
  const leftRule = left[quality];
  const rightRule = right[quality];
  return Boolean(leftRule?.enabled !== false) === Boolean(rightRule?.enabled !== false) && Number(leftRule?.creditCost || 0) === Number(rightRule?.creditCost || 0);
});
var getSupportedQualities = (modelId) => {
  const caps = getModelCapabilities(modelId);
  const supportedSizes = caps?.supportedSizes || ["1K" /* SIZE_1K */, "2K" /* SIZE_2K */, "4K" /* SIZE_4K */];
  const supportedQualities = supportedSizes.map((size) => SIZE_TO_QUALITY[size]).filter((quality) => !!quality);
...
var AdvancedToggle = ({
  ...
  "button",
  {
    ...
    onClick: onToggle,
    "data-tone": tone,
    className: `credit-advanced-switch ${checked ? "is-on" : ""} ${size === "compact" ? "credit-advanced-switch--compact" : ""}`,
    children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "sr-only", children: label }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-advanced-switch__thumb" })
    ]
  }
);
var CreditModelSettings = () => {
  const [rows, setRows] = (0, import_react3.useState)([]);
  const [loading, setLoading] = (0, import_react3.useState)(false);
  const [saving, setSaving] = (0, import_react3.useState)(false);
  const [supportsMaxCallsLimit, setSupportsMaxCallsLimit] = (0, import_react3.useState)(true);
  const [supportsAdvancedSettings, setSupportsAdvancedSettings] = (0, import_react3.useState)(true);
  const [selectedProviderId, setSelectedProviderId] = (0, import_react3.useState)("");
  const [form, setForm] = (0, import_react3.useState)(emptyProvider());
  const [cachedPricing, setCachedPricing] = (0, import_react3.useState)(null);
  const [pricingCacheStatus, setPricingCacheStatus] = (0, import_react3.useState)("idle");
  const [pricingMultiplier, setPricingMultiplier] = (0, import_react3.useState)(1);
  const providers = (0, import_react3.useMemo)(() => {
    const grouped = /* @__PURE__ */ new Map();
  ...
  (0, import_react3.useEffect)(() => {
    ...
    void getCachedPricing(providerId).then((data) => {
      if (cancelled) return;
  ...
  const vendorProfile = (0, import_react3.useMemo)(
  ...
  const modelMixSnapshots = (0, import_react3.useMemo)(() => {
    ...
    const providerNameMap = /* @__PURE__ */ new Map();
    const routeProvidersByBaseId = /* @__PURE__ */ new Map();
    rows.filter((row) => row.provider_id !== currentProviderId).forEach((row) => {
      const baseId = normalizeBaseModelId(row.model_id);
      if (!baseId) return;
      if (!routeProvidersByBaseId.has(baseId)) {
        routeProvidersByBaseId.set(baseId, /* @__PURE__ */ new Set());
      }
  ...
  const creditsPerUsd = (0, import_react3.useMemo)(
    () => Math.max(1, DEFAULT_CREDITS_PER_USD * pricingMultiplier),
    [pricingMultiplier]
  );
  const modelSuggestions = (0, import_react3.useMemo)(
    () => form.models.map((model, index) => {
      ...
      const qualitySuggestionChanged = supportsAdvancedSettings && !areQualityPricingEqual(model.qualityPricing, suggestion.recommendedQualityPricing, supportedQualities);
      return {
        index,
        baseModelId,
        supportedQualities,
        ...
        hasSuggestedChange: suggestion.recommendedCredits !== Number(model.creditCost || 1) || qualitySuggestionChanged
      };
    }),
    [form.models, cachedPricing, creditsPerUsd, modelMixSnapshots, supportsAdvancedSettings]
  );
  const pricingCacheSummary = (0, import_react3.useMemo)(() => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from("admin_credit_models").select(CREDIT_MODEL_SELECT_WITH_LIMIT).order("provider_id", { ascending: true }).order("priority", { ascending: false });
      ...
      if (error && missingColumns.length > 0) {
        const fallbackSelect = getCreditModelSelect({
          ...
          includeAdvancedSettings: !missingColumns.some(
            (column) => ["advanced_enabled", "mix_with_same_model", "quality_pricing"].includes(column)
          )
        });
        const legacy = await supabase.from("admin_credit_models").select(fallbackSelect).order("provider_id", { ascending: true }).order("priority", { ascending: false });
        ...
        setSupportsAdvancedSettings(
          !missingColumns.includes("advanced_enabled") && !missingColumns.includes("mix_with_same_model") && !missingColumns.includes("quality_pricing")
        );
        notify.warning(
          "\u5DF2\u542F\u7528\u517C\u5BB9\u6A21\u5F0F",
  ...
  const loadProviderRows = async (providerId) => {
    const { data, error } = await supabase.from("admin_credit_models").select(CREDIT_MODEL_SELECT_BASE).eq("provider_id", providerId).order("priority", { ascending: false });
    if (error) throw error;
    return data || [];
  };
  const needsDirectStyleRepair = (savedRows, payloadModels) => {
    const rowMap = new Map(
      savedRows.map((row) => [normalizeBaseModelId(row.model_id), row])
    );
  ...
  const saveProviderDirect = async (payloadModels) => {
    const providerId = form.providerId.trim();
    const providerName = form.providerName.trim();
    const baseUrl = form.baseUrl.trim();
    const apiKey = form.apiKey.trim();
    const { error: deleteError } = await supabase.from("admin_credit_models").delete().eq("provider_id", providerId);
    ...
    const { error: insertError } = await supabase.from("admin_credit_models").insert(rowsToInsert);
    if (insertError) throw insertError;
  };
  const refreshAdminModelSync = async () => {
    const [{ adminModelService: adminModelService2 }, { unifiedModelService: unifiedModelService2 }] = await Promise.all([
      Promise.resolve().then(() => (init_adminModelService(), adminModelService_exports)),
      Promise.resolve().then(() => (init_unifiedModelService(), unifiedModelService_exports))
    ]);
    await adminModelService2.forceLoadAdminModels();
    await unifiedModelService2.refreshModels();
  };
  (0, import_react3.useEffect)(() => {
  ...
  const updateModelAt = (index, patch) => {
    setForm((prev) => ({
      ...prev,
      models: prev.models.map((item, i) => i === index ? { ...item, ...patch } : item)
    }));
  };
  const updateModelQualityAt = (index, quality, patch) => {
    setForm((prev) => ({
      ...prev,
  ...
  const applySuggestionToModel = (index) => {
    const entry = modelSuggestions[index];
    if (!entry) return;
    setForm((prev) => ({
      ...prev,
      models: prev.models.map(
        (item, i) => i === index ? {
          ...item,
          creditCost: entry.suggestion.recommendedCredits,
          qualityPricing: supportsAdvancedSettings ? entry.suggestion.recommendedQualityPricing : item.qualityPricing
        } : item
      )
    }));
  };
  ...
  const saveProvider = async () => {
    ...
    try {
      const payloadModels = validModels.map((item, index) => ({
        ...
        quality_pricing: ADMIN_MODEL_QUALITY_KEYS.reduce((acc, quality) => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "space-y-4", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "settings-intent-card settings-intent-card--warning", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "settings-intent-card__title", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ShieldAlert, { className: "h-4 w-4" }),
        "\u79EF\u5206\u6A21\u578B\u914D\u7F6E\uFF08\u5168\u5C40\uFF09"
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { className: "settings-intent-card__body", children: "\u8FD9\u91CC\u914D\u7F6E\u7684\u662F\u7BA1\u7406\u5458\u5168\u5C40\u79EF\u5206\u6A21\u578B\uFF0C\u4F1A\u540C\u6B65\u7ED9\u6240\u6709\u7528\u6237\u3002\u7528\u6237\u81EA\u5DF1\u7684\u63A5\u53E3\u914D\u7F6E\u4E0D\u5728\u672C\u9875\u9762\u4FEE\u6539\u3002" })
    ] }),
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "grid grid-cols-1 gap-4 xl:grid-cols-[280px,minmax(0,1fr)]", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "settings-section-card self-start p-3 xl:sticky xl:top-4", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mb-2 text-xs text-[var(--text-tertiary)]", children: "\u5DF2\u914D\u7F6E\u4F9B\u5E94\u5546" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "settings-scroll-region space-y-2 pr-1", children: providers.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "apple-empty-state rounded-lg border border-dashed border-[var(--border-light)] p-3 text-xs text-[var(--text-tertiary)]", children: "\u6682\u65E0\u79EF\u5206\u4F9B\u5E94\u5546\u914D\u7F6E\u3002" }) : providers.map((item) => {
          ...
          return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", { onClick: () => setSelectedProviderId(item.providerId), className: "w-full text-left", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-sm text-[var(--text-primary)]", children: first.provider_name }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-[11px] text-[var(--text-tertiary)]", children: item.providerId }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-1 text-[11px] text-[var(--text-tertiary)]", children: [
                    "\u6A21\u578B\u603B\u6570\uFF1A",
                    item.items.length,
                    " | \u542F\u7528\uFF1A",
                    activeCount
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "settings-section-card space-y-4 p-4", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mb-2 text-xs font-semibold text-[var(--text-primary)]", children: "\u4F9B\u5E94\u5546\u57FA\u7840\u4FE1\u606F" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "grid grid-cols-1 gap-3 md:grid-cols-2", children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u4F9B\u5E94\u5546 \u7F16\u53F7\uFF08\u552F\u4E00\uFF09" }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                "input",
                {
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u4F9B\u5E94\u5546\u540D\u79F0" }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                "input",
                {
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u57FA\u7840 \u5730\u5740" }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                "input",
                {
                  value: form.baseUrl,
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u4E0A\u6E38 \u63A5\u53E3\u5BC6\u94A5" }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                "input",
                {
                  type: "password",
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "grid grid-cols-1 gap-3 xl:grid-cols-2", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "rounded-xl border border-[var(--border-light)] bg-[var(--bg-secondary)] p-3", children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-start justify-between gap-3", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-xs font-semibold text-[var(--text-primary)]", children: "\u5382\u5BB6\u8BC6\u522B / \u6765\u6E90\u9694\u79BB" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-1 text-[11px] leading-5 text-[var(--text-tertiary)]", children: "\u5F53\u524D\u9875\u9762\u53EA\u7BA1\u7406\u7BA1\u7406\u5458\u5168\u5C40\u79EF\u5206\u8DEF\u7531\uFF0C\u4E0D\u4F1A\u4FEE\u6539\u7528\u6237\u81EA\u5DF1\u6DFB\u52A0\u7684 API\u3001\u5BC6\u94A5\u548C\u6A21\u578B\u5217\u8868\u3002" })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "rounded-full border border-indigo-500/20 bg-indigo-500/10 px-2 py-1 text-[10px] font-medium text-indigo-300", children: vendorProfile.confidence === "high" ? "\u9AD8\u7F6E\u4FE1" : vendorProfile.confidence === "medium" ? "\u4E2D\u7F6E\u4FE1" : "\u4F4E\u7F6E\u4FE1" })
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "rounded-lg bg-[var(--bg-tertiary)] px-3 py-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-[10px] uppercase tracking-[0.08em] text-[var(--text-tertiary)]", children: "\u8BC6\u522B\u5382\u5BB6" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-1 text-sm font-semibold text-[var(--text-primary)]", children: vendorProfile.label })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "rounded-lg bg-[var(--bg-tertiary)] px-3 py-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-[10px] uppercase tracking-[0.08em] text-[var(--text-tertiary)]", children: "\u534F\u8BAE\u65CF" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-1 text-sm font-semibold text-[var(--text-primary)]", children: vendorProfile.endpointFamily === "gemini" ? "Gemini" : "OpenAI Compatible" })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "rounded-lg bg-[var(--bg-tertiary)] px-3 py-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-[10px] uppercase tracking-[0.08em] text-[var(--text-tertiary)]", children: "\u53EF\u6DF7\u5408\u6A21\u578B" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-1 text-sm font-semibold text-[var(--text-primary)]", children: [
                  mixEligibleCount,
                  " / ",
                  form.models.length
                ] })
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-3 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-[11px] leading-5 text-emerald-200", children: vendorProfile.hint }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-3 space-y-2", children: modelMixSnapshots.filter((item) => item.baseModelId).length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "rounded-lg border border-dashed border-[var(--border-light)] px-3 py-2 text-[11px] text-[var(--text-tertiary)]", children: "\u5F55\u5165\u6A21\u578B\u7F16\u53F7\u540E\uFF0C\u8FD9\u91CC\u4F1A\u81EA\u52A8\u63D0\u793A\u54EA\u4E9B\u6A21\u578B\u53EF\u4EE5\u4E0E\u5176\u4ED6\u4F9B\u5E94\u5546\u505A\u540C ID \u6DF7\u5408\u8C03\u914D\u3002" }) : modelMixSnapshots.filter((item) => item.baseModelId).map((item) => {
                  ...
                  children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex flex-wrap items-center justify-between gap-2", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "min-w-0", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-sm font-medium text-[var(--text-primary)]", children: model.displayName || item.baseModelId }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-1 text-[11px] text-[var(--text-tertiary)]", children: [
                          "\u6A21\u578B ID\uFF1A",
                          item.baseModelId
                        ] })
                      ...
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                        "span",
                        {
                          className: `rounded-full px-2 py-1 text-[10px] font-medium ${item.peerProviderCount > 0 ? "bg-emerald-500/14 text-emerald-200" : "bg-[var(--bg-elevated)] text-[var(--text-tertiary)]"}`,
                          children: item.peerProviderCount > 0 ? `\u53EF\u6DF7\u5408 ${item.routeCount} \u6761\u8DEF\u7531` : "\u5F53\u524D\u4EC5\u5355\u8DEF\u7531"
                        }
                      )
                    ] }),
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-2 text-[11px] leading-5 text-[var(--text-secondary)]", children: item.peerProviderCount > 0 ? `\u5339\u914D\u5230 ${item.peerProviderCount} \u4E2A\u5176\u4ED6\u4F9B\u5E94\u5546\uFF1A${item.peerProviders.join("\u3001")}\u3002\u5F00\u542F\u201C\u591A\u4F9B\u5E94\u5546\u6DF7\u5408\u201D\u540E\u5373\u53EF\u53C2\u4E0E\u667A\u80FD\u8C03\u914D\u3002` : "\u6682\u672A\u53D1\u73B0\u5176\u4ED6\u4F9B\u5E94\u5546\u5B58\u5728\u540C model_id\uFF0C\u4E0D\u4F1A\u8FDB\u5165\u6DF7\u5408\u8DEF\u7531\u6C60\u3002" })
                  ]
                },
                `${item.baseModelId}-${item.index}`
              );
            }) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "rounded-xl border border-[var(--border-light)] bg-[var(--bg-secondary)] p-3", children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-start justify-between gap-3", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-xs font-semibold text-[var(--text-primary)]", children: "\u4EF7\u683C\u5EFA\u8BAE / \u5FEB\u901F\u8C03\u8282" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-1 text-[11px] leading-5 text-[var(--text-tertiary)]", children: "\u4F9D\u636E\u7F13\u5B58\u4EF7\u683C\u3001\u5185\u7F6E\u5B9A\u4EF7\u548C\u79EF\u5206\u6362\u7B97\u7CFB\u6570\uFF0C\u5FEB\u901F\u56DE\u586B\u6BCF\u4E2A\u6A21\u578B\u7684\u63A8\u8350\u79EF\u5206\u3002" })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                "button",
                {
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-3 grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr),120px]", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "rounded-lg bg-[var(--bg-tertiary)] px-3 py-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "text-[11px] font-medium text-[var(--text-primary)]", children: [
                  "$1 \u2248 ",
                  DEFAULT_CREDITS_PER_USD,
                  " \u79EF\u5206 \xD7 \u7CFB\u6570 = ",
                  creditsPerUsd.toFixed(1),
                  " \u79EF\u5206"
                ] }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-1 text-[11px] leading-5 text-[var(--text-tertiary)]", children: pricingCacheSummary })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u8C03\u8282\u7CFB\u6570" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "input",
                  {
                    type: "number",
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-3 space-y-2", children: modelSuggestions.filter((item) => item.baseModelId).length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "rounded-lg border border-dashed border-[var(--border-light)] px-3 py-2 text-[11px] text-[var(--text-tertiary)]", children: "\u6DFB\u52A0\u6A21\u578B\u540E\uFF0C\u8FD9\u91CC\u4F1A\u81EA\u52A8\u663E\u793A\u5F53\u524D\u79EF\u5206\u4E0E\u5EFA\u8BAE\u79EF\u5206\u7684\u5BF9\u7167\u3002" }) : modelSuggestions.filter((item) => item.baseModelId).map((item) => {
              const model = form.models[item.index];
              const qualityPreview = item.supportedQualities.map(
                (quality) => `${quality}: ${item.suggestion.recommendedQualityPricing[quality].creditCost}`
              ).join(" / ");
              return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex flex-wrap items-start justify-between gap-2", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "min-w-0 flex-1", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-sm font-medium text-[var(--text-primary)]", children: model.displayName || item.baseModelId }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-1 text-[11px] leading-5 text-[var(--text-secondary)]", children: [
                          "\u5F53\u524D ",
                          model.creditCost,
                          " \u79EF\u5206 \u2192 \u5EFA\u8BAE ",
                          item.suggestion.recommendedCredits,
                          " \u79EF\u5206"
                        ] }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "text-[11px] leading-5 text-[var(--text-tertiary)]", children: [
                          "\u6765\u6E90\uFF1A",
                          item.suggestion.sourceLabel,
                          item.suggestion.usdEstimate !== null ? ` \xB7 \u4F30\u7B97\u5355\u6B21\u6210\u672C ${formatUsdEstimate(item.suggestion.usdEstimate)}` : "",
                          item.suggestion.matchedModel ? ` \xB7 \u5339\u914D ${item.suggestion.matchedModel}` : ""
                        ] })
                      ] }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                        "button",
                    ...
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-2 text-[11px] leading-5 text-[var(--text-tertiary)]", children: item.suggestion.note }),
                    supportsAdvancedSettings && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mt-2 flex flex-wrap gap-1.5", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "rounded-full bg-indigo-500/14 px-2 py-1 text-[10px] font-medium text-indigo-200", children: "\u753B\u8D28\u77E9\u9635\u5EFA\u8BAE" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { className: "text-[10px] leading-5 text-[var(--text-tertiary)]", children: [
                        qualityPreview,
                        " \u79EF\u5206"
                      ] })
                    ] })
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "space-y-3", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-xs font-semibold text-[var(--text-primary)]", children: "\u6A21\u578B\u914D\u7F6E" }),
          !supportsMaxCallsLimit && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "settings-intent-card settings-intent-card--warning px-3 py-2 text-[11px]", children: "\u5F53\u524D\u6570\u636E\u5E93\u672A\u5305\u542B\u603B\u8C03\u7528\u4E0A\u9650\u5B57\u6BB5\uFF08`max_calls_limit`\uFF09\uFF0C\u5DF2\u81EA\u52A8\u964D\u7EA7\u517C\u5BB9\u3002 \u6267\u884C\u6700\u65B0 Supabase \u8FC1\u79FB\u540E\uFF0C\u53EF\u542F\u7528\u201C\u603B\u8C03\u7528\u4E0A\u9650/\u81EA\u52A8\u6682\u505C\u201D\u80FD\u529B\u3002" }),
          !supportsAdvancedSettings && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "settings-intent-card settings-intent-card--warning px-3 py-2 text-[11px]", children: "\u5F53\u524D\u6570\u636E\u5E93\u672A\u5305\u542B\u9AD8\u7EA7\u8BBE\u7F6E\u5B57\u6BB5\uFF08`advanced_enabled / mix_with_same_model / quality_pricing`\uFF09\uFF0C \u5DF2\u81EA\u52A8\u9690\u85CF\u753B\u8D28\u5B9A\u4EF7\u4E0E\u6DF7\u5408\u8DEF\u7531\u914D\u7F6E\u3002\u6267\u884C\u6700\u65B0 Supabase \u8FC1\u79FB\u540E\u5373\u53EF\u542F\u7528\u3002" }),
          form.models.map((model, index) => {
            ...
            return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "rounded-xl border border-[var(--border-light)] p-3 space-y-2", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "grid grid-cols-1 gap-2 md:grid-cols-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u6A21\u578B\u7F16\u53F7" }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "input",
                    {
                      value: model.modelId,
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u663E\u793A\u540D\u79F0" }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "input",
                    {
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u63A5\u53E3\u7C7B\u578B" }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                    "select",
                    {
                      ...
                      className: "w-full rounded-lg border border-[var(--border-light)] bg-[var(--bg-tertiary)] px-3 py-2 text-sm",
                      children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "auto", children: "\u81EA\u52A8\u5224\u65AD" }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "openai", children: "\u901A\u7528\u517C\u5BB9\u63A5\u53E3" }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "gemini", children: "\u8C37\u6B4C\u63A5\u53E3" })
                      ]
                    }
                  )
                ] }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: [
                    "\u79EF\u5206\u6D88\u8017",
                    model.advancedEnabled && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "ml-1.5 text-[10px] text-indigo-400", children: "(\u9AD8\u7EA7\u6A21\u5F0F)" })
                  ] }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "input",
                    {
                      ...
                      onChange: (e) => {
                        ...
                        setForm((prev) => ({
                          ...prev,
                          models: prev.models.map((item, i) => i === index ? {
                            ...item,
                            creditCost: value,
                  ...
                  model.advancedEnabled && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mt-1.5 flex flex-wrap gap-1.5", children: qualitiesToShow.map((quality) => {
                    ...
                    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                      "span",
                      {
                        className: `inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[10px] font-medium ${isEnabled ? "bg-indigo-500/14 text-indigo-500 dark:text-indigo-200" : "bg-[var(--bg-elevated)] text-[var(--text-tertiary)]"}`,
                        children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: qualityMeta.resolution }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
                            rule.creditCost,
                            " \u79EF\u5206"
                          ] }),
                          !isEnabled ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "opacity-80", children: "\u5DF2\u505C\u7528" }) : null
                        ]
                      },
                      quality
                    );
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u4E3B\u989C\u8272" }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "input",
                    {
                      type: "color",
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u526F\u989C\u8272\uFF08\u53EF\u9009\uFF09" }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "input",
                    {
                      type: "color",
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u6587\u672C\u989C\u8272" }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                    "select",
                    {
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u603B\u8C03\u7528\u4E0A\u9650\uFF08\u7559\u7A7A\u4E3A\u65E0\u9650\uFF09" }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "input",
                    {
                      type: "number",
                      ...
                      onChange: (e) => {
                        ...
                        setForm((prev) => ({
                          ...prev,
                          models: prev.models.map(
                            (item, i) => i === index ? { ...item, maxCallsLimit: value && value > 0 ? value : null } : item
                          )
                        }));
                      },
                      placeholder: "\u9ED8\u8BA4\u65E0\u9650",
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "space-y-1 block", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[11px] text-[var(--text-tertiary)]", children: "\u63CF\u8FF0\uFF08\u53EF\u9009\uFF09" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "input",
                  {
              ...
              supportsAdvancedSettings && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-panel", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-panel__header", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-panel__intro", children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-panel__icon", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SlidersHorizontal, { size: 16 }) }),
                    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "min-w-0", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-panel__eyebrow", children: "Advanced Controls" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-panel__title-row", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-panel__title", children: "\u9AD8\u7EA7\u8BBE\u7F6E" }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: `credit-advanced-panel__status ${model.advancedEnabled ? "is-on" : ""}`, children: model.advancedEnabled ? "\u5DF2\u542F\u7528" : "\u672A\u542F\u7528" })
                      ] }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-panel__description", children: "\u628A\u590D\u6742\u914D\u7F6E\u6536\u655B\u6210\u4E24\u4EF6\u4E8B\uFF1A\u6DF7\u5408\u8DEF\u7531\u548C\u5206\u8FA8\u7387\u5B9A\u4EF7\u3002" })
                    ] })
                  ] }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-panel__control", children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-panel__control-label", children: "\u542F\u7528\u9AD8\u7EA7\u7B56\u7565" }),
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                      AdvancedToggle,
                      {
                ...
                model.advancedEnabled && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-panel__body", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-grid", children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card__head", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-card__icon-badge credit-advanced-card__icon-badge--emerald", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowRightLeft, { size: 15 }) }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-card__title", children: "\u591A\u4F9B\u5E94\u5546\u6DF7\u5408" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-card__description", children: "\u81EA\u52A8\u5747\u8861\u540C\u6A21\u578B\u4E0B\u7684\u8BF7\u6C42\u91CF\uFF0C\u4F18\u5148\u4F7F\u7528\u8C03\u7528\u6B21\u6570\u8F83\u5C11\u7684\u4F9B\u5E94\u5546\u3002" })
                        ] })
                      ] }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card__footer", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: `credit-advanced-card__pill ${model.mixWithSameModel ? "is-on" : ""}`, children: model.mixWithSameModel ? "\u81EA\u52A8\u5747\u8861\u5DF2\u5F00\u542F" : "\u4FDD\u6301\u5355\u4F9B\u5E94\u5546" }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                          AdvancedToggle,
                          {
                    ...
                    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card__head", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-card__icon-badge credit-advanced-card__icon-badge--indigo", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Sparkles, { size: 15 }) }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-card__title", children: "\u753B\u8D28\u5B9A\u4EF7" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-advanced-card__description", children: "\u6309\u5206\u8FA8\u7387\u5355\u72EC\u8BBE\u7F6E\u79EF\u5206\u6210\u672C\uFF0C\u65B9\u4FBF\u628A\u9AD8\u8D28\u91CF\u51FA\u56FE\u4E0E\u9ED8\u8BA4\u8D39\u7387\u533A\u5206\u5F00\u3002" })
                        ] })
                      ] }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card__stats", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card__stat", children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-advanced-card__stat-label", children: "\u53EF\u914D\u7F6E\u89C4\u683C" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-advanced-card__stat-value", children: qualitiesToShow.length })
                        ] }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card__stat", children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-advanced-card__stat-label", children: "\u5F53\u524D\u542F\u7528" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-advanced-card__stat-value", children: enabledQualityCount })
                        ] }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-card__stat", children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-advanced-card__stat-label", children: "\u9ED8\u8BA4\u79EF\u5206" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-advanced-card__stat-value", children: model.creditCost })
                        ] })
                      ] })
                    ] })
                  ] }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-quality-section", children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-quality-section__header", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-quality-section__eyebrow", children: "Pricing Matrix" }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-quality-section__title", children: "\u6309\u753B\u8D28\u5355\u72EC\u5B9A\u4EF7" })
                      ] }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-quality-section__summary", children: [
                        "\u5DF2\u542F\u7528 ",
                        enabledQualityCount,
                        " / ",
                        qualitiesToShow.length
                      ] })
                    ] }),
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-quality-grid", children: qualitiesToShow.map((quality) => {
                      const rule = model.qualityPricing[quality];
                      const isEnabled = rule.enabled !== false;
                      const qualityMeta = QUALITY_META[quality];
                      return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                        "div",
                        {
                          ...
                          children: [
                            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-quality-card__top", children: [
                              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-quality-card__label-row", children: [
                                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-quality-card__name", children: quality }),
                                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-quality-card__resolution", children: qualityMeta.resolution })
                                ] }),
                                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "credit-quality-card__hint", children: qualityMeta.hint })
                              ] }),
                              /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                                AdvancedToggle,
                                {
                            ...
                            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "credit-quality-card__field", children: [
                              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-quality-card__field-label", children: isEnabled ? "\u5355\u6B21\u79EF\u5206" : "\u5F53\u524D\u5DF2\u505C\u7528" }),
                              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-quality-card__input-wrap", children: [
                                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                                  "input",
                                  {
                                    type: "number",
                                    ...
                                    onChange: (e) => updateModelQualityAt(index, quality, {
                                      creditCost: Math.max(1, Number(e.target.value || model.creditCost || 1))
                                    }),
                                    className: "credit-quality-input"
                                  }
                                ),
                                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "credit-quality-card__suffix", children: "\u79EF\u5206" })
                              ] })
                            ] })
                          ]
                        },
                  ...
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "credit-advanced-note", children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Info, { className: "mt-0.5 h-4 w-4 shrink-0" }),
                    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "text-[11px] leading-5", children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "font-semibold", children: "\u6DF7\u5408\u8DEF\u7531\u7B56\u7565\uFF1A" }),
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "text-[11px] text-[var(--text-tertiary)]", children: [
                  "\u5DF2\u8C03\u7528\uFF1A",
                  callsUsed ?? 0,
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-4", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "text-xs text-[var(--text-secondary)]", children: [
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                      "input",
                      {
                        className: "mr-2",
                  ...
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { onClick: () => removeModel(index), className: "settings-danger-text text-xs", children: "\u5220\u9664\u6A21\u578B" })
                ] })
              ] })
            ] }, `${model.modelId}-${index}`);
          })
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "settings-action-row", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", { onClick: addModel, className: "apple-button-secondary min-h-10 px-3 text-xs", children: [
...
var CreditModelSettings_default = CreditModelSettings;
export {
  CreditModelSettings_default as default
};
