...
function extractSlotRouteTarget(suffix) {
  ...
  if (decodedSuffix.startsWith("slot_key_")) return decodedSuffix.slice(9);
...
function buildProviderRouteId(baseModelId, providerId) {
  return `${String(baseModelId || "").trim()}@provider_${encodeURIComponent(String(providerId || "").trim())}`;
}
    ...
    KeyManager = class {
      ...
      getNextKey(modelId, preferredKeyId) {
        ...
        const matchesRequestedRoute = (slot) => {
          ...
          const decodedSuffix = (() => {
            try {
              return decodeURIComponent(normalizedSuffix);
            } catch {
          ...
          if (routeTarget && slotIdLower === routeTarget) return true;
          if (slotNameLower === decodedSuffix) return true;
          if (slotSuffixLower === decodedSuffix) return true;
          if (providerLower === decodedSuffix) return true;
          if (slotNameLower.includes(decodedSuffix) || slotSuffixLower.includes(decodedSuffix)) return true;
          return false;
        };
        if (preferredKeyId) {
      ...
      hasCustomKeyForModel(modelIdFull) {
        ...
        const hasValidSlot = this.state.slots.some((s) => {
          ...
          if (suffix) {
            ...
            if (routeTarget && slotIdLower === routeTarget) {
              return true;
            }
            if (slotNameLower === decodedSuffix || slotSuffixLower === decodedSuffix || providerLower === decodedSuffix) {
              return true;
            }
          }
          return false;
        ...
        return this.providers.some((p) => {
          ...
          if (suffix) {
            ...
            const providerIdLower = String(p.id || "").trim().toLowerCase();
            if (routeTarget && providerIdLower === routeTarget) return true;
            if (providerNameLower === decodedSuffix) return true;
            if (providerNameLower.includes(decodedSuffix)) return true;
          }
          return false;
        });
      }
