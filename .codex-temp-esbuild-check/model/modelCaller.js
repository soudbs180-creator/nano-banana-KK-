...
// src/services/model/modelCaller.ts
init_apiConfig();
init_providerStrategy();
init_keyManager();
...
// src/services/model/secureModelProxy.ts
init_supabase();
...
async function callSecureSystemProxyChat(payload) {
...
// src/services/model/modelCaller.ts
var ModelCaller = class {
  isModelMatch(modelId, candidate) {
    const normalizedModelId = String(modelId || "").trim().toLowerCase();
    const normalizedCandidate = String(candidate || "").trim().toLowerCase();
    if (!normalizedModelId || !normalizedCandidate) {
      return false;
    }
    return normalizedCandidate === normalizedModelId || normalizedCandidate.endsWith(`/${normalizedModelId}`) || normalizedModelId.endsWith(`/${normalizedCandidate}`);
  }
  findConfiguredProviderForModel(modelId) {
    const providers = keyManager.getProviders().filter((provider) => provider.isActive && provider.baseUrl && provider.apiKey);
  ...
  async call(options) {
    ...
    const userSlot = slots.find(
      (slot) => slot.supportedModels?.includes(modelId) || slot.supportedModels?.some((supportedModel) => modelId.includes(supportedModel))
    );
    if (userSlot) {
  ...
  async callCreditModel(options, creditCost) {
    const {
      data: { user }
  ...
  async callViaSupplier(options, supplier) {
    return this.callWithProtocol(options, supplier);
  }
  async callWithUserKey(options, userKey) {
  ...
  async callWithProtocol(options, config) {
    const runtime = resolveProviderRuntime({
  ...
  async callOpenAICompatible(options, config) {
    try {
      const runtime = resolveProviderRuntime({
        provider: config.provider,
        baseUrl: config.baseUrl,
      ...
      const response = await fetch(buildOpenAIEndpoint(config.baseUrl, "chat/completions"), {
        method: "POST",
        headers: buildProxyHeaders(runtime.authMethod, config.apiKey, runtime.headerName, void 0, runtime.authorizationValueFormat),
        body: JSON.stringify({
  ...
  buildGeminiPayload(options) {
    const systemInstruction = options.messages.filter((message) => message.role === "system").map((message) => String(message.content || "").trim()).filter(Boolean).join("\n\n");
    const contents = options.messages.filter((message) => message.role !== "system").map((message) => ({
  ...
  async callGeminiCompatible(options, config) {
    try {
      const runtime = resolveProviderRuntime({
        provider: config.provider,
        baseUrl: config.baseUrl,
  ...
  async callWithSystemDefault(options) {
  ...
  async deductCredits(userId, credits, modelId) {
...
var modelCaller = new ModelCaller();
export {
  modelCaller
};
