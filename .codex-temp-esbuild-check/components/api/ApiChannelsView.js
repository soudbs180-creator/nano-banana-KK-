...
// src/components/api/ApiChannelsView.tsx
...
var Key = createLucideIcon("key", __iconNode8);
...
var Pause = createLucideIcon("pause", __iconNode10);
...
var Server = createLucideIcon("server", __iconNode14);
...
var splitModelStrings = (input) => {
  if (!input || !input.trim()) return [];
  const cleanInput = input.replace(/[\u{1F300}-\u{1F9FF}]/gu, "").replace(/(图像|视频|聊天|其他)\s*:/g, "").replace(/(image|video|chat|other)\s*:/gi, "");
  return cleanInput.split(/[,\n]/).map((s) => s.trim()).filter(Boolean).map((s) => parseModelString(s).id);
};
var DEFAULT_GOOGLE_MODELS2 = [
  // 图片模型
  "gemini-2.5-flash-image",
  "gemini-3-pro-image-preview",
  "imagen-4.0-generate-001",
...
var ApiChannelsView = ({ mode = "dispatch" }) => {
  const [slots, setSlots] = (0, import_react3.useState)(keyManager_default.getSlots());
  const [strategy, setStrategy] = (0, import_react3.useState)(keyManager_default.getStrategy());
  const clampAndFormat = (value) => {
    const v = Math.floor(value || 0);
    return v.toLocaleString("en-US");
  };
  const [loading, setLoading] = (0, import_react3.useState)(false);
  const [refreshingIds, setRefreshingIds] = (0, import_react3.useState)(/* @__PURE__ */ new Set());
  const [draggedId, setDraggedId] = (0, import_react3.useState)(null);
  ...
  const [formName, setFormName] = (0, import_react3.useState)("");
  const [formProvider, setFormProvider] = (0, import_react3.useState)("Google");
  const [formKey, setFormKey] = (0, import_react3.useState)("");
  const [formBaseUrl, setFormBaseUrl] = (0, import_react3.useState)("");
  const [formImageModels, setFormImageModels] = (0, import_react3.useState)("");
  const [formVideoModels, setFormVideoModels] = (0, import_react3.useState)("");
  const [formChatModels, setFormChatModels] = (0, import_react3.useState)("");
  const [formOtherModels, setFormOtherModels] = (0, import_react3.useState)("");
  const [formCompatibility, setFormCompatibility] = (0, import_react3.useState)("standard");
  const [expandedType, setExpandedType] = (0, import_react3.useState)(null);
  const [formBudgetLimit, setFormBudgetLimit] = (0, import_react3.useState)(-1);
  const [formCreditCost, setFormCreditCost] = (0, import_react3.useState)(void 0);
  const [isKeyEditing, setIsKeyEditing] = (0, import_react3.useState)(false);
  const detectProviderFromKey = (key) => {
    ...
    if (k.startsWith("AIza")) {
      ...
      if (preset) {
        ...
        setFormCompatibility(preset.mode);
      }
    }
  };
  const detectSettingsFromUrl = (url) => {
    const runtime = resolveProviderRuntime({ baseUrl: url });
    setFormProvider(runtime.uiProvider);
  ...
  const [testStatus, setTestStatus] = (0, import_react3.useState)("idle");
  const [testMessage, setTestMessage] = (0, import_react3.useState)("");
  (0, import_react3.useEffect)(() => {
    const update = () => {
      const allSlots = keyManager_default.getSlots();
      ...
      setStrategy(keyManager_default.getStrategy());
    };
    const unsub = keyManager_default.subscribe(update);
    return unsub;
  }, []);
  const handleStrategyChange = (newStrategy) => {
    keyManager_default.setStrategy(newStrategy);
  ...
  const openEditModal = (slot) => {
    ...
    setFormChatModels(categorized.chatModels.join(", "));
    setFormOtherModels(categorized.otherModels?.join(", ") || "");
    setFormBudgetLimit(slot.budgetLimit ?? -1);
    setFormCreditCost(slot.creditCost);
    setTestStatus("idle");
    setTestMessage("");
    setIsModalOpen(true);
  ...
  const handleToggle = (id, e) => {
    e?.stopPropagation();
    keyManager_default.toggleKey(id);
  };
  const handleRefresh = async (id, e) => {
    ...
    if (slot) {
      const targetUrl = slot.baseUrl || "https://generativelanguage.googleapis.com";
      ...
      if (successTest) {
        keyManager_default.reportSuccess(id);
        notify.success("\u8FDE\u63A5\u6210\u529F", successTest.message || "");
      } else {
        const errorMsg = failTest?.message || "\u8FDE\u63A5\u5931\u8D25";
        keyManager_default.reportFailure(id, errorMsg);
  ...
  const handleDrop = (e) => {
    e.preventDefault();
    if (draggedId) {
      const finalFromIndex = keyManager_default.getSlots().findIndex((s) => s.id === draggedId);
      const finalToIndex = slots.findIndex((s) => s.id === draggedId);
      if (finalFromIndex !== -1 && finalToIndex !== -1 && finalFromIndex !== finalToIndex) {
        keyManager_default.reorderSlots(finalFromIndex, finalToIndex);
      }
    }
    setDraggedId(null);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (finalModels.length === 0 && (formProvider === "Google" || !formBaseUrl)) {
      finalModels = [...DEFAULT_GOOGLE_MODELS2];
    }
    const keyData = {
      name: formName.trim() || "API Channel",
      key: formKey.trim(),
      provider: formProvider,
      // Cast to any or Provider to fix type error
    ...
    let result;
    if (editingId) {
      await keyManager_default.updateKey(editingId, keyData);
    } else {
      result = await keyManager_default.addKey(formKey.trim(), keyData);
    }
    ...
    Promise.resolve().then(() => (init_costService(), costService_exports)).then(({ forceSync: forceSync2 }) => {
      forceSync2().then(() => console.log("[ApiChannelsView] Cost service synced"));
    });
    setLoading(false);
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex flex-col h-full overflow-hidden", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center justify-between flex-wrap gap-4 px-1 py-4 shrink-0", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", { className: "text-2xl font-bold text-left", style: { color: "var(--text-primary)" }, children: "API \u4FE1\u9053" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { className: "text-xs mt-1", style: { color: "var(--text-tertiary)" }, children: isSequential ? "\u987A\u5E8F\u4F18\u5148: \u6309\u5217\u8868\u987A\u5E8F\u4F9D\u6B21\u8C03\u7528" : "\u5E76\u53D1\u4F18\u5148: \u968F\u673A/\u8D1F\u8F7D\u5747\u8861\u8C03\u7528" })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bg-[var(--bg-tertiary)] border border-[var(--border-light)] rounded-xl h-10 p-1 flex items-center", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "button",
            {
              onClick: () => handleStrategyChange("round-robin"),
              className: `px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${strategy === "round-robin" ? "bg-[var(--bg-primary)] text-[var(--text-primary)] shadow-sm border border-[var(--border-light)]" : "text-[var(--text-tertiary)] hover:text-[var(--text-primary)]"}`,
              title: "\u968F\u673A/\u8D1F\u8F7D\u5747\u8861",
              children: "\u5E76\u53D1\u4F18\u5148"
            }
          ),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "button",
            {
              ...
              className: `px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${strategy === "sequential" ? "bg-[var(--bg-primary)] text-[var(--text-primary)] shadow-sm border border-[var(--border-light)]" : "text-[var(--text-tertiary)] hover:text-[var(--text-primary)]"}`,
              title: "\u987A\u5E8F\u4F18\u5148",
              children: "\u987A\u5E8F\u4F18\u5148"
            }
          )
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
          "button",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "hidden sm:inline", children: "\u6DFB\u52A0\u4FE1\u9053" }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "sm:hidden", children: "\u6DFB\u52A0" })
            ]
          }
        )
      ] })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex-1 overflow-y-auto px-1 pb-4 min-h-0 custom-scrollbar", children: slots.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "h-full flex flex-col items-center justify-center text-[var(--text-tertiary)] space-y-4", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-16 h-16 rounded-2xl bg-[var(--bg-tertiary)]/50 flex items-center justify-center", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Key, { className: "w-8 h-8 opacity-50" }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "\u6682\u65E0 API \u4FE1\u9053" }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "button",
        {
          onClick: openAddModal,
    ...
    ] }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex flex-col gap-8 px-4 pb-6 pt-2 max-w-3xl mx-auto", children: [
      {
        title: "\u5B98\u65B9\u76F4\u8FDE (Official)",
        keys: officialGoogleKeys,
        icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Zap, { size: 14 }),
      ...
      },
      {
        title: "\u4EE3\u7406\u670D\u52A1\u5668 (Proxy)",
        keys: proxyKeys,
        icon: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Server, { size: 14 }),
        colorClass: "text-emerald-500",
        // Different color for Proxy
    ...
    ].map((section) => section.keys.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex flex-col gap-3", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2 px-1", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: `p-1.5 rounded-lg ${section.bgClass} ${section.colorClass}`, children: section.icon }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h4", { className: "text-sm font-bold opacity-80", style: { color: "var(--text-secondary)" }, children: section.title })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex flex-col gap-3", children: section.keys.map((slot, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
        "div",
        {
          draggable: true,
          ...
          className: `
                                                    ...
                                                    ${slot.disabled ? "bg-[var(--bg-tertiary)]/30 border-[var(--border-medium)]/50 opacity-60" : `bg-[var(--bg-secondary)] border-[var(--border-light)] hover:border-${section.colorClass.split("-")[1]}-500/30 hover:shadow-lg hover:shadow-${section.colorClass.split("-")[1]}-500/5`}
                                                    p-4 w-full
                                                ...
                                                `,
          children: [
            isSequential && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute top-4 left-4 shrink-0 w-6 h-6 rounded-full bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-xs font-mono text-indigo-400 font-semibold", children: index + 1 }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex-1 min-w-0", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: `min-w-0 ${isSequential ? "pl-8" : ""}`, children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center justify-between mb-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2 min-w-0", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: `w-2 h-2 rounded-full shrink-0 ${refreshingIds.has(slot.id) ? "bg-blue-500 animate-pulse" : slot.disabled ? "bg-[var(--text-tertiary)]" : slot.status === "valid" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : slot.status === "invalid" || slot.status === "rate_limited" ? "bg-red-500" : "bg-[var(--text-tertiary)]"}` }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h4", { className: "text-base font-semibold truncate pr-2", style: { color: "var(--text-primary)" }, title: slot.name, children: slot.name })
                ] }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: `text-[10px] px-1.5 py-0.5 rounded border font-medium ${slot.provider === "Google" ? "bg-blue-500/10 text-blue-500 border-blue-500/20" : "bg-purple-500/10 text-purple-500 border-purple-500/20"}`, children: slot.provider })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2 text-xs font-mono", style: { color: "var(--text-tertiary)" }, children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Key, { size: 12 }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "truncate", children: maskApiKey(slot.key) })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-3 mt-2 pt-2 border-t border-[var(--border-light)]/50", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex items-center gap-1.5 text-xs", children: refreshingIds.has(slot.id) ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Clock, { className: "text-blue-400 animate-pulse", size: 12 }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { color: "#60a5fa" }, children: "\u68C0\u6D4B\u4E2D" })
                ] }) : slot.disabled ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Pause, { size: 12 }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { color: "var(--text-tertiary)" }, children: "\u5DF2\u7981\u7528" })
                ] }) : slot.status === "valid" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Check, { className: "text-emerald-500", size: 12 }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { color: "#10b981" }, children: "\u5728\u7EBF" })
                ] }) : slot.status === "rate_limited" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Clock, { className: "text-orange-500", size: 12 }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { color: "#f59e0b" }, children: "\u9650\u6D41" })
                ] }) : slot.status === "invalid" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { className: "text-red-500", size: 12 }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { color: "#ef4444" }, children: "\u79BB\u7EBF" })
                ] }) : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Activity, { size: 12, style: { color: "var(--text-tertiary)" } }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { color: "var(--text-tertiary)" }, children: "\u672A\u77E5" })
                ] }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex-1" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "button",
                    {
                      ...
                      title: "\u6D4B\u8BD5\u8FDE\u63A5",
                      children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RefreshCw, { size: 14, className: refreshingIds.has(slot.id) ? "animate-spin" : "" })
                    }
                  ),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "button",
                    {
                      ...
                      children: slot.disabled ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Play, { size: 14 }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Pause, { size: 14 })
                    }
                  ),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    "button",
    ...
    isModalOpen && (0, import_react_dom.createPortal)(
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "fixed inset-0 z-[10050] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bg-[var(--bg-secondary)] w-full max-w-md rounded-2xl border border-[var(--border-light)] shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] overflow-hidden", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex justify-between items-center p-5 border-b border-[var(--border-light)]", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h4", { className: "text-lg font-bold text-[var(--text-primary)]", children: editingId ? "\u7F16\u8F91\u4FE1\u9053" : "\u6DFB\u52A0\u4FE1\u9053" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { onClick: () => setIsModalOpen(false), className: "text-[var(--text-tertiary)] hover:text-[var(--text-primary)]", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { size: 20 }) })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "p-6 space-y-5 overflow-y-auto custom-scrollbar", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "space-y-4", children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex items-center justify-between text-xs text-[var(--text-tertiary)] uppercase tracking-wider font-bold", children: "\u8FDE\u63A5\u914D\u7F6E" }),
            formProvider !== "Google" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex justify-between items-center mb-1.5", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { className: "text-xs text-[var(--text-secondary)]", children: "\u63A5\u53E3\u5730\u5740 (Base URL)" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                  "select",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "", children: "\u{1F680} \u5FEB\u901F\u9009\u62E9..." }),
                      PRESET_PROVIDERS.filter((p) => p.provider !== "Google").map((p) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: p.label, children: p.label }, p.label))
                    ]
                  }
                )
              ] }),
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "input",
                  {
                    ...
                    onChange: (e) => {
                      ...
                      if (val.includes("deepseek")) {
                        setFormName((n) => n === "New Channel" || !n ? "DeepSeek" : n);
                      } else if (val.includes("silicon")) {
                        setFormName((n) => n === "New Channel" || !n ? "SiliconFlow" : n);
                      } else if (val.includes("openrouter")) {
                        setFormName((n) => n === "New Channel" || !n ? "OpenRouter" : n);
                      }
                    }
                  }
                ),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Globe, { className: "absolute left-3 top-2.5 text-[var(--text-secondary)] pointer-events-none", size: 14 })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex justify-end mt-2", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                "button",
                {
                  type: "button",
                  onClick: handleTestConnection,
                  disabled: testStatus === "testing",
                  className: `text - [10px] px - 3 py - 1.5 rounded border flex items - center gap - 2 transition - colors ${testStatus === "success" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : testStatus === "error" ? "bg-red-500/10 text-red-500 border-red-500/20" : "bg-[var(--bg-tertiary)] text-[var(--text-tertiary)] border-[var(--border-medium)] hover:text-[var(--text-secondary)]"} `,
                  children: [
                    testStatus === "testing" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Globe, { size: 12, className: "animate-spin" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Terminal, { size: 12 }),
                    testStatus === "testing" ? "\u6D4B\u8BD5\u4E2D..." : "\u6D4B\u8BD5\u8FDE\u63A5 (Test Connection)"
                  ]
                }
              ) }),
              testMessage && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: `text - [10px] mt - 2 p - 2 rounded text - right ${testStatus === "success" ? "text-emerald-400" : testStatus === "error" ? "text-red-400" : "text-[var(--text-tertiary)]"} `, children: testMessage })
            ] }),
            formProvider !== "Google" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bg-[var(--bg-primary)] rounded px-3 py-2 border border-[var(--border-light)] flex items-center justify-between", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex flex-col", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[10px] text-[var(--text-secondary)] uppercase font-bold tracking-wider", children: "\u81EA\u52A8\u9002\u914D\u6A21\u5F0F" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-xs text-[var(--text-secondary)]", children: formCompatibility === "standard" ? "\u6807\u51C6 API (Standard)" : "\u5BF9\u8BDD\u6A21\u62DF (Chat Compat)" })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "text-[10px] text-[var(--text-secondary)] max-w-[50%] text-right", children: formCompatibility === "standard" ? "\u9002\u7528\u4E8E\u5927\u591A\u6570\u517C\u5BB9 OpenAI \u7684\u4E2D\u8F6C\u7AD9" : "\u9002\u7528\u4E8E Cherry Studio\u3001SiliconFlow \u7B49\u7279\u6B8A\u6E20\u9053" })
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { className: "text-xs text-[var(--text-secondary)] mb-1.5 block", children: "API Key" }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "relative", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "input",
                  {
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Key, { className: "absolute left-3 top-2.5 text-[var(--text-secondary)] pointer-events-none", size: 14 }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "button",
                  {
                    type: "button",
                    ...
                    children: isKeyEditing ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EyeOff, { size: 14 }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Eye, { size: 14 })
                  }
                )
              ] })
            ] }),
...
\u{1F4F8} \u56FE\u7247: ${formImageModels.split(",").filter(Boolean).length}\u4E2A
\u{1F3AC} \u89C6\u9891: ${formVideoModels.split(",").filter(Boolean).length}\u4E2A
\u{1F4AC} \u804A\u5929: ${formChatModels.split(",").filter(Boolean).length}\u4E2A`
                          );
                        }
                      } else {
...
\u{1F4F8} \u56FE\u50CF: ${imageModels.length}\u4E2A
\u{1F3AC} \u89C6\u9891: ${videoModels.length}\u4E2A
\u{1F4AC} \u804A\u5929: ${chatModels.length}\u4E2A${otherModels && otherModels.length > 0 ? `
\u{1F527} \u5176\u4ED6: ${otherModels.length}\u4E2A` : ""}`
                          );
                        } else {
                          const isNewApiLikely = formBaseUrl && (formBaseUrl.includes("newapi") || formBaseUrl.includes("oneapi") || formBaseUrl.includes("gemini-api.cn") || formBaseUrl.includes("vodeshop"));
                          const hasExistingModels = formImageModels && formImageModels.length > 0 || formVideoModels && formVideoModels.length > 0 || formChatModels && formChatModels.length > 0;
...
\u{1F4A1} \u63D0\u793A\uFF1ANewAPI \u670D\u52A1\u5546\u8BF7\u5C1D\u8BD5\u4ECE\u201C\u6A21\u578B\u5E7F\u573A\u201D\u7684\u201C\u4F9B\u5E94\u5546\u201D\u4E2D\u83B7\u53D6\u914D\u7F6E\u3002`
                        );
                      } else {
                  ...
                  children: [
                    loading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LoaderCircle, { size: 16, className: "animate-spin" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Zap, { size: 16 }),
                    loading ? "\u68C0\u6D4B\u4E2D..." : "\u667A\u80FD\u68C0\u6D4B\u6A21\u578B (Auto Detect Models)"
                  ]
                }
              ),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { className: "text-[10px] text-[var(--text-tertiary)] mt-1.5", children: "\u{1F4A1} \u63D0\u793A: \u81EA\u52A8\u68C0\u6D4B\u4F1A\u5C1D\u8BD5\u8FDE\u63A5\u63A5\u53E3\u5E76\u83B7\u53D6\u53EF\u7528\u6A21\u578B\u5217\u8868\uFF0C\u5E76\u81EA\u52A8\u586B\u5145\u5230\u4E0B\u65B9\u3002" })
            ] })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "space-y-4", children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex items-center justify-between text-xs text-[var(--text-tertiary)] uppercase tracking-wider font-bold", children: "\u57FA\u7840\u4FE1\u606F & \u6A21\u578B" }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "grid grid-cols-2 gap-4", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { className: "text-xs text-[var(--text-secondary)] mb-1.5 block", children: "\u4FE1\u9053\u540D\u79F0" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "input",
                  {
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { className: "text-xs text-[var(--text-secondary)] mb-1.5 block", children: "\u4F9B\u5E94\u5546\u7C7B\u578B" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                  "select",
                  {
                    className: "w-full bg-[var(--bg-tertiary)] border border-[var(--border-light)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-indigo-500/50",
                    ...
                    children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "Google", children: "Google / Gemini" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "OpenAI", children: "OpenAI Compatible" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "Anthropic", children: "Anthropic" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "Aliyun", children: "Aliyun (DashScope)" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "Tencent", children: "Tencent (Hunyuan)" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "Volcengine", children: "Volcengine (Doubao)" }),
                      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "SiliconFlow", children: "SiliconFlow" }),
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "space-y-3", children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flex items-center justify-between", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { className: "text-xs text-[var(--text-secondary)]", children: "\u53EF\u7528\u6A21\u578B\u5217\u8868 (\u6309\u7C7B\u578B\u5206\u7EC4)" }) }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "grid grid-cols-2 gap-3", children: [
                  ...
                  "div",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center justify-between", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2 text-xs font-bold text-[var(--text-secondary)]", children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u{1F4F8}" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u56FE\u50CF\u6A21\u578B (Image)" })
                        ] }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                          "button",
                          {
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                  "div",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center justify-between", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2 text-xs font-bold text-[var(--text-secondary)]", children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u{1F3AC}" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u89C6\u9891\u6A21\u578B (Video)" })
                        ] }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                          "button",
                          {
                    ...
                    children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center justify-between", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2 text-xs font-bold text-[var(--text-secondary)]", children: [
                          ...
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u804A\u5929/\u63A8\u7406 (Chat)" })
                        ] }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                          "button",
                          {
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                  "div",
                  {
                    onClick: (e) => {
                      e.stopPropagation();
                      setExpandedType("other");
                    },
                    className: "bg-[var(--bg-tertiary)] border border-[var(--border-light)] rounded-lg p-3 flex flex-col gap-2 relative group cursor-pointer hover:border-indigo-500/30 transition-all",
                    children: [
                      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center justify-between", children: [
                        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2 text-xs font-bold text-[var(--text-secondary)]", children: [
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u{1F527}" }),
                          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u5176\u4ED6/\u591A\u6A21\u6001 (Other)" })
                        ] }),
                        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                          "button",
                          {
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { className: "text-[10px] text-[var(--text-tertiary)]", children: "\u{1F4A1} \u63D0\u793A: \u70B9\u51FB\u4E0A\u65B9\u201C\u667A\u80FD\u68C0\u6D4B\u6A21\u578B\u201D\u4F1A\u81EA\u52A8\u586B\u5145\u5230\u5BF9\u5E94\u5206\u7C7B" })
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "text-xs text-[var(--text-secondary)] mb-1.5 flex justify-between items-center", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u{1F4B0} \u9884\u7B97\u9650\u5236 (Budget Limit)" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[10px] text-[var(--text-tertiary)]", children: formBudgetLimit < 0 ? "\u267E\uFE0F \u65E0\u9650\u5236" : `$${formBudgetLimit.toFixed(2)}` })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DollarSign, { size: 14, className: "text-[var(--text-secondary)]" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "input",
                  {
                    type: "number",
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-xs text-[var(--text-tertiary)]", children: "USD" })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { className: "text-[10px] text-[var(--text-tertiary)] mt-1.5 flex items-start gap-1", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u{1F4A1}" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "-1 = \u65E0\u9650\u5236 | >0 = \u8FBE\u5230\u9884\u7B97\u540E\u81EA\u52A8\u505C\u7528\u6B64\u4FE1\u9053" })
              ] })
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { className: "text-xs text-[var(--text-secondary)] mb-1.5 flex justify-between items-center", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u26A1 \u5355\u6B21\u8C03\u7528\u4F30\u8BA1\u8D39\u7528/\u6D88\u8017\u6210\u672C (User API Cost)" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "text-[10px] text-[var(--text-tertiary)]", children: formCreditCost === void 0 ? "\u{1F193} \u514D\u8D39/\u4E0D\u6263\u8D39" : `$${formCreditCost}` })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex items-center gap-2", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DollarSign, { size: 14, className: "text-[var(--text-secondary)]" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "input",
                  {
                    ...
                    min: "0",
                    className: "flex-1 bg-[var(--bg-tertiary)] border border-[var(--border-light)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] font-mono outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/20 transition-all",
                    placeholder: "\u7559\u7A7A\u5373\u4E3A\u514D\u8D39(0)",
                    value: formCreditCost === void 0 ? "" : formCreditCost,
                    onChange: (e) => {
                      const val = e.target.value;
                      if (val === "") {
                        setFormCreditCost(void 0);
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { className: "text-[10px] text-[var(--text-tertiary)] mt-1.5 flex items-start gap-1", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u{1F4A1}" }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u8FD9\u662F\u7528\u6237\u914D\u7F6E\u7684\u6A21\u578B\u72EC\u7ACB\u8D39\u7528\u9884\u4F30\uFF0C\u6B64 API \u4EA7\u751F\u7684\u8C03\u7528\u5C06\u7EDF\u8BA1\u5230\u60A8\u7684\u603B\u652F\u51FA\uFF0C**\u4E0D\u4F1A\u6263\u9664\u7CFB\u7EDF\u516C\u5171\u79EF\u5206**\u3002" })
              ] })
            ] })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "p-5 border-t border-[var(--border-light)] flex justify-end gap-3 bg-[var(--bg-secondary)]", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "button",
            {
          ...
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "button",
            {
              ...
              children: loading ? "\u5904\u7406\u4E2D..." : editingId ? "\u4FDD\u5B58\u4FEE\u6539" : "\u6DFB\u52A0\u4FE1\u9053"
            }
          )
        ] })
      ] }) }),
    ...
    expandedType && (0, import_react_dom.createPortal)(
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bg-[var(--bg-secondary)] w-full max-w-2xl rounded-xl shadow-2xl border border-[var(--border-light)] flex flex-col max-h-[80vh] m-4 animate-in zoom-in-95 duration-200", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "p-4 border-b border-[var(--border-light)] flex items-center justify-between", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h3", { className: "font-bold flex items-center gap-2", children: [
            expandedType === "image" && "\u{1F4F8} \u7F16\u8F91\u56FE\u50CF\u6A21\u578B",
            expandedType === "video" && "\u{1F3AC} \u7F16\u8F91\u89C6\u9891\u6A21\u578B",
            expandedType === "chat" && "\u{1F4AC} \u7F16\u8F91\u804A\u5929\u6A21\u578B",
            expandedType === "other" && "\u{1F527} \u7F16\u8F91\u5176\u4ED6\u6A21\u578B"
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { onClick: () => setExpandedType(null), className: "p-1 hover:bg-[var(--bg-tertiary)] rounded-full transition-colors", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { size: 20, className: "text-[var(--text-secondary)]" }) })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flex-1 p-4 flex flex-col min-h-0", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { className: "text-xs text-[var(--text-tertiary)] mb-2", children: '\u6BCF\u884C\u4E00\u4E2AID\uFF0C\u6216\u7528\u9017\u53F7\u5206\u9694\u3002\u652F\u6301 "ID (\u522B\u540D)" \u683C\u5F0F\u3002' }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "textarea",
            {
              ...
              value: expandedType === "image" ? formImageModels : expandedType === "video" ? formVideoModels : expandedType === "chat" ? formChatModels : formOtherModels,
              onChange: (e) => {
                const val = e.target.value;
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "p-4 border-t border-[var(--border-light)] flex justify-end", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          "button",
          {
