...
当前 `v1.3.6` 版本以 **API 设置体验重构、亮暗主题一致性、价格信息精简展示与整体稳定性修复** 为核心目标，重点强化：
- API 设置面板改为新版双栏卡片结构，默认直达第三方供应商
- 第三方供应商编辑区完整显示，避免旧版遮挡、挤压和竖排文本
- 价格扫描结果只保留品牌供应商、分组、计费方式与最终价格/倍率
- 继续清理亮暗模式下的颜色反转、对比度不足和局部显示异常
...
### 配置 API
1. 点击左下角 **Settings / 系统设置**
2. 进入 **接口管理**
3. 添加 Google Gemini Key 或 NewAPI / OpenAI Compatible 渠道
4. 开始创作！
...
#### 当前基线
- **API 设置页重构**：设置外壳与接口管理页统一为新版双卡片布局，默认打开第三方供应商，编辑目标更清晰
- **第三方供应商优化**：左侧选择列表支持搜索、放宽宽度并强制横向排版，解决文字被挤压、URL 竖排和卡片遮挡问题
- **价格扫描展示收口**：高级模式下仅展示品牌供应商、分组、计费方式和价格结果；按次计费支持直接展示尺寸倍率换算结果
- **主题一致性修复**：继续修正亮色/暗色模式下的底色、文字、边框和按钮对比度，降低白底白字、暗底暗字等问题
- **版本说明同步**：README 当前版本说明更新为以新版 API 设置体验与界面修复为主的 `1.3.6` 口径

#### 当前建议
- 优先以 README 中的“当前版本说明”作为 `v1.3.6` 的对外说明口径
- 历史更新日志继续保留用于追溯，不再代表当前界面与交互基线
