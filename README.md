...
当前 `v1.3.6` 版本以 **稳定性、版本统一、移动端可用性、存储可靠性与部署一致性** 为核心目标，重点强化：
- 统一版本元信息与说明文档
- 双层存储保护与本地恢复链路
- 设置面板与关键模块的懒加载和构建分块
- API / 计费 / 画布相关问题的巡检与修复
...
#### 当前基线
- **统一版本源**：前端展示、主文档、开发文档、子服务版本统一到 `1.3.6`
- **版本展示收口**：画布版本标识改为统一常量维护，避免多处手写不一致
- **说明文档更新**：README、交接文档、进度文档、移动端报告、`.agent` 规则同步到最新口径
- **工程规范增强**：新增 `.agent` 版本同步与修改核验规则，降低后续改动跑偏概率

#### 当前建议
- 优先使用文档中的“当前版本说明”作为项目状态依据
- 历史更新日志保留用于追溯，不再代表当前基线
